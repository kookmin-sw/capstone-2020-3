...
export class LocalDatePipe implements PipeTransform {
  ...
  transform(value: string, displayFormat: string) {
    ...
    if(value){
      ...
      if(displayFormat){
        changeDate = this.datePipe.transform(new Date(Date.UTC(year, month, day, hour, minute, second)), displayFormat);
      }else{
        changeDate = this.datePipe.transform(new Date(Date.UTC(year, month, day, hour, minute, second)), dftDisplayFormat);
      }
